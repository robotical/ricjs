...
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
import { RICWifiConnState, RICWifiConnStatus, } from "./RICWifiTypes";
import RICLog from "./RICLog";
import { RICConfiguredAddOns, RICCalibInfo, RICFileList, RICFriendlyName, RICHWElemList_Str, RICOKFail, RICSystemInfo, } from "./RICTypes";
export default class RICSystem {
    ...
    _systemInfo = null;
    // RIC naming
    _ricFriendlyName = null;
    // HWElems (connected to RIC) - excluding AddOns
    _hwElemsExcludingAddOns = new Array();
    _connectedAddOns = new Array();
    // Calibration info
    _calibInfo = null;
    // WiFi connection info
    _ricWifiConnStatus = new RICWifiConnStatus();
    ...
    async retrieveInfo() {
        ...
        try {
            ...
            RICLog.debug(`retrieveInfo - RIC Version ${this._systemInfo?.SystemVersion}`);
        }
        catch (error) {
    ...
    // Mark: Calibration -----------------------------------------------------------------------------------------
    async calibrate(cmd, jointList, jointNames) {
        ...
        if (cmd === "set") {
            ...
            for (const jnt of jointList) {
                try {
                    ...
                    await this._ricMsgHandler.sendRICRESTURL(saveCalibCmd);
                    if (rsl.rslt != "ok")
                        overallResult = false;
                }
                catch (error) {
            ...
            try {
                ...
                if (hwElemList && hwElemList.rslt && hwElemList.rslt === "ok") {
                    ...
                    if (reqType === "RSAddOn") {
                        ...
                        // Debug
                        RICLog.debug(`getHWElemList: found ${hwElemList.hw.length} addons/buspixels`);
                    }
                        ...
                        // Debug
                        RICLog.debug(`getHWElemList: found ${hwElemList.hw.length} elems matching ${reqType}`);
                    }
                }
            }
            catch (error) {
    ...
    async getSysModInfoBLEMan() {
        try {
            ...
            // Debug
            RICLog.debug(`getSysModInfoBLEMan rslt ${bleInfo.rslt} isConn ${bleInfo.isConn} paused ${bleInfo.isAdv} txBPS ${bleInfo.txBPS} rxBPS ${bleInfo.rxBPS}`);
            // Check for test rate
            if ("tBPS" in bleInfo) {
                RICLog.debug(`getSysModInfoBLEMan testMsgs ${bleInfo.tM} testBytes ${bleInfo.tB} testRateBytesPS ${bleInfo.tBPS}`);
            }
            return bleInfo;
        }
        catch (error) {
    ...
    async getWiFiConnStatus() {
        try {
            ...
            RICLog.debug(`wifiConnStatus rslt ${ricSysModInfoWiFi.rslt} isConn ${ricSysModInfoWiFi.isConn} paused ${ricSysModInfoWiFi.isPaused}`);
