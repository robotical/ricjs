...
export default class RICUpdateManager {
    ...
    _progressDuringUpload = 0.8;
    _progressDuringRestart = 0.015;
    // there may be two restarts during an update
    _progressAfterUpload = this._progressAfterDownload + this._progressDuringUpload + 2 * this._progressDuringRestart;
    // TESTS - set to true for testing OTA updates ONLY
    TEST_TRUNCATE_ESP_FILE = false;
    TEST_PRETEND_ELEM_UPDATE_REQD = false;
    TEST_PRETEND_INITIAL_VERSIONS_DIFFER = false; // this is public so it can be set from the front-end to force an update
    TEST_PRETEND_FINAL_VERSIONS_MATCH = false;
    TEST_SKIP_FW_UPDATE = false;
    constructor(_ricMsgHandler, _ricFileHandler, _ricSystem, _eventListener, _firmwareTypeStrForMainFw, _nonFirmwareElemTypes, _currentAppVersion, _fileDownloader, _firmwareUpdateURL) {
        this._ricMsgHandler = _ricMsgHandler;
        this._ricFileHandler = _ricFileHandler;
        this._ricSystem = _ricSystem;
        this._eventListener = _eventListener;
        this._firmwareTypeStrForMainFw = _firmwareTypeStrForMainFw;
        this._nonFirmwareElemTypes = _nonFirmwareElemTypes;
    ...
    async checkForUpdate(systemInfo) {
        ...
        try {
            ...
            RICLog.debug(`checkForUpdate systemVersion ${systemInfo?.SystemVersion} available online ${this._latestVersionInfo?.firmwareVersion} updateRequired ${updateRequired}`);
            if (updateRequired) {
                if (RICUtils.isVersionGreater(this._latestVersionInfo.minimumUpdaterVersion.ota, this._currentAppVersion)) {
    ...
    async _isUpdateRequired(latestVersion, systemInfo) {
        ...
        this._updateESPRequired = RICUtils.isVersionGreater(latestVersion.firmwareVersion, systemInfo.SystemVersion);
        ...
        if (!this._updateESPRequired) {
            try {
                const elUpdRslt = await this._ricMsgHandler.sendRICRESTURL('hwfwupd');
                // Check result
                ...
                RICLog.debug('isUpdateRequired failed to get hw-elem firmware update status');
            }
        }
        else {
            this._updateElemsRequired = true;
    ...
    async firmwareUpdate() {
        ...
        try {
            for (let fwIdx = 0; fwIdx < firmwareList.length; fwIdx++) {
                ...
                const downloadResult = await this._fileDownloader(firmwareList[fwIdx].downloadUrl, (received, total) => {
                    ...
                    this._eventListener(RICUpdateEvent.UPDATE_PROGRESS, { stage: 'Downloading firmware', progress: currentProgress, updatingFilesystem: false });
                });
            ...
            for (let fwIdx = 0; fwIdx < firmwareData.length; fwIdx++) {
                ...
                if (elemType == RICFileSendType.RIC_FIRMWARE_UPDATE && this.TEST_SKIP_FW_UPDATE) {
                    ...
                    await this.fileSend(firmwareList[fwIdx].destname, elemType, firmwareData[fwIdx], (_, __, progress) => {
                        ...
                        if (percComplete > 1.0)
                            percComplete = 1.0;
                        RICLog.debug(`fwUpdate progress ${progress.toFixed(2)} sent ${sentBytes} len ${firmwareData[fwIdx].length} total ${totalBytes} propComplete ${percComplete.toFixed(2)}`);
                        this._eventListener(RICUpdateEvent.UPDATE_PROGRESS, {
                            stage: updateStage,
                            progress: percComplete,
                            updatingFilesystem: updatingFilesystem,
        ...
        for (const elemFw of firmwareList) {
            ...
            if (elemFw.elemType === this._firmwareTypeStrForMainFw)
                continue;
            // Non-firmware elemTypes
            if (this._nonFirmwareElemTypes.includes(elemFw.elemType))
                continue;
            // Start hw-elem update
            const updateCmd = `hwfwupd/${elemFw.elemType}/${elemFw.destname}/all`;
            try {
                await this._ricMsgHandler.sendRICRESTURL(updateCmd);
            }
            catch (error) {
    ...
    async waitForRestart(percComplete, checkFwVersion = null) {
        ...
        for (let fwUpdateCheckCount = 0; fwUpdateCheckCount < this.FW_UPDATE_CHECKS_BEFORE_ASSUME_FAILED; fwUpdateCheckCount++) {
            try {
                ...
                RICLog.debug(`fwUpdate version rslt "${systemInfo.rslt}" RIC Version ${systemInfo.SystemVersion}`);
                if (systemInfo.rslt !== 'ok') {
                    continue;
                }
                if (checkFwVersion != null) {
                    // Check version
                    versionConfirmed = RICUtils.isVersionEqual(checkFwVersion, systemInfo.SystemVersion);
