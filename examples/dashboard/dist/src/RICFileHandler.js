...
export default class RICFileHandler {
    ...
    _requestedFileBlockSize = 500;
    _fileBlockSize = 0;
    _requestedBatchAckSize = 10;
    _batchAckSize = 0;
    ...
    async _sendFileStartMsg(fileName, fileType, fileContents) {
        ...
        try {
            fileStartResp = await this._msgHandler.sendRICREST(cmdMsg, RICRESTElemCode.RICREST_ELEM_CODE_COMMAND_FRAME);
        }
        catch (err) {
        ...
        RICLog.debug(`_fileSendStartMsg fileBlockSize req ${this._requestedFileBlockSize} resp ${fileStartResp.batchMsgSize} actual ${this._fileBlockSize}`);
        RICLog.debug(`_fileSendStartMsg batchAckSize req ${this._requestedBatchAckSize} resp ${fileStartResp.batchAckSize} actual ${this._batchAckSize}`);
        return true;
    }
    async _sendFileEndMsg(fileName, fileType, fileContents) {
        ...
        try {
            fileEndResp = await this._msgHandler.sendRICREST(cmdMsg, RICRESTElemCode.RICREST_ELEM_CODE_COMMAND_FRAME);
        }
        catch (err) {
    ...
    async _sendFileCancelMsg() {
        ...
        // Send
        try {
            return await this._msgHandler.sendRICREST(cmdMsg, RICRESTElemCode.RICREST_ELEM_CODE_COMMAND_FRAME);
        }
        catch (err) {
    ...
    async _sendFileContents(fileContents, progressCallback) {
        ...
        while (this._ackedFilePos < fileContents.length) {
            ...
            if (this._sendWithoutBatchAcks) {
                // Debug
                RICLog.verbose(`_sendFileContents NO BATCH ACKS ${progressUpdateCtr} blocks total sent ${this._ackedFilePos} block len ${this._fileBlockSize}`);
            ...
            if ((progressUpdateCtr >= 20) && progressCallback) {
                ...
                // Debug
                RICLog.debug(`_sendFileContents ${progressUpdateCtr} blocks sent OkTo ${this._ackedFilePos} block len ${this._fileBlockSize}`);
                // Continue
                progressUpdateCtr = 0;
            }
        }
