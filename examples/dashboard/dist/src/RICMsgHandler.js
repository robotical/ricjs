...
import { RICROSSerial, } from './RICROSSerial';
import { PROTOCOL_RICREST, RICSERIAL_MSG_NUM_POS, RICSERIAL_PAYLOAD_POS, RICSERIAL_PROTOCOL_POS, RICREST_REST_ELEM_CODE_POS, RICREST_HEADER_PAYLOAD_POS, } from './RICProtocolDefs';
...
export var RICCommsMsgTypeCode;
...
})(RICCommsMsgTypeCode || (RICCommsMsgTypeCode = {}));
export var RICCommsMsgProtocol;
...
})(RICCommsMsgProtocol || (RICCommsMsgProtocol = {}));
// Message results
export var RICMsgResultCode;
...
export default class RICMsgHandler {
    ...
    _onHDLCFrameDecode(rxMsg) {
        ...
        if (rxProtocol == PROTOCOL_RICREST) {
            RICLog.verbose(`_onHDLCFrameDecode RICREST rx msgNum ${rxMsgNum} msgDirn ${rxMsgType} ${RICUtils.bufferToHex(rxMsg)}`);
            // Extract payload
            ...
            if (ricRestElemCode == RICRESTElemCode.RICREST_ELEM_CODE_URL ||
                ...
                RICLog.verbose(`_onHDLCFrameDecode RICREST rx elemCode ${ricRestElemCode} ${restStr}`);
                // Check message types
                ...
                RICLog.debug(`_onHDLCFrameDecode RICREST rx binary message elemCode ${ricRestElemCode} len ${binMsgLen}`);
            }
        }
    ...
    _handleResponseMessages(restStr, rxMsgNum) {
        try {
            ...
            if ('rslt' in msgRsltJsonObj) {
                ...
                if (rsltStr === 'ok') {
                    RICLog.verbose(`_handleResponseMessages RICREST rslt Ok ${rxMsgNum == 0 ? "unnumbered" : "msgNum " + rxMsgNum.toString()} resp ${msgRsltJsonObj.rslt}`);
                    msgRsltCode = RICMsgResultCode.MESSAGE_RESULT_OK;
                }
                else if (rsltStr === 'fail') {
                    msgRsltCode = RICMsgResultCode.MESSAGE_RESULT_FAIL;
                    RICLog.warn(`_handleResponseMessages RICREST rslt fail ${rxMsgNum == 0 ? "unnumbered" : "msgNum " + rxMsgNum.toString()} resp ${restStr}`);
                }
                else {
                    RICLog.warn(`_handleResponseMessages RICREST rslt not recognized ${rxMsgNum == 0 ? "unnumbered" : "msgNum " + rxMsgNum.toString()}resp ${restStr}`);
                }
            }
            else {
                RICLog.warn(`_handleResponseMessages RICREST response doesn't contain rslt ${rxMsgNum == 0 ? "unnumbered" : "msgNum " + rxMsgNum.toString()}resp ${restStr}`);
            }
            ...
            if (excp instanceof Error) {
                RICLog.warn(`_handleResponseMessages Failed to parse JSON ${rxMsgNum == 0 ? "unnumbered" : "msgNum " + rxMsgNum.toString()} JSON STR ${restStr} resp ${excp.toString()}`);
            }
        }
    }
    _handleReportMessages(restStr) {
        try {
            ...
            if (excp instanceof Error) {
                RICLog.warn(`_handleReportMessages Failed to parse JSON report ${excp.toString()}`);
            }
        }
    }
    ...
    msgTrackingTxCmdMsg(msgFrame, withResponse, msgTimeoutMs, resolve, reject) {
        ...
        // Debug
        RICLog.verbose(`msgTrackingTxCmdMsg msgNum ${this._currentMsgNumber} msg ${RICUtils.bufferToHex(msgFrame)} sanityCheck ${this._msgTrackInfos[this._currentMsgNumber].msgOutstanding}`);
        // RICLog.debug(
    ...
    msgTrackingRxRespMsg(msgNum, msgRsltCode, msgRsltJsonObj) {
        ...
        // Handle message
        RICLog.verbose(`msgTrackingRxRespMsg Message response received msgNum ${msgNum}`);
            ...
            for (let loopIdx = 0; loopIdx < this._msgTrackInfos.length; loopIdx++) {
                ...
                if ((this._msgTrackInfos[checkIdx].retryCount === 0) || (Date.now() > this._msgTrackInfos[checkIdx].msgSentMs + msgTimeoutMs)) {
                    ...
                    if (this._msgTrackInfos[checkIdx].retryCount < RICMsgTrackInfo.MSG_RETRY_COUNT) {
                        ...
                        RICLog.warn(`msgTrackTimer TIMEOUT msgNum ${checkIdx} after ${RICMsgTrackInfo.MSG_RETRY_COUNT} retries`);
    ...
    encodeFileStreamBlock(blockContents, blockStart, streamID) {
        ...
        const msgBuf = new Uint8Array(blockContents.length + 4 + RICREST_HEADER_PAYLOAD_POS + RICSERIAL_PAYLOAD_POS);
        let msgBufPos = 0;
        // RICSERIAL protocol
