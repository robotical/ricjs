...
export function checkNewData(name, data) {
    return true;
}
export async function sysInfoGet(params) {
...
export async function connPerfTest(params) {
    try {
...
export async function setReconnect(params) {
    if (globalThis.ricConnector.isConnected()) {
        globalThis.ricConnector.setRetryConnectionIfLost(params[0], params[1]);
    }
}
export function robotStatusFormat(name, robotStatus) {
...
export function powerStatusFormat(name, powerStatus) {
...
export function imuStatusFormat(name, imuStatus) {
...
export function servoStatusFormat(name, servoStatus) {
...
export function addonListFormat(name, addons) {
...
export function addonValListFormat(name, addons) {
    if (!checkNewData(name, addons)) {
        return "";
    }
    let statusStr = "";
    ...
    for (let i = 0; i < addons.length; i++) {
        ...
        for (const valKey in addon.vals) {
            if (typeof addon.vals[valKey] === "number")
                addon.vals[valKey] = Math.round(+addon.vals[valKey]);
        }
        // @ts-ignore
...
export function tableFormat(infoStr, infoObj, headings = undefined) {
    let statusStr = "";
    if (infoStr.length > 0) {
...
export function commsStatusFormat(name, commsStats) {
