...
function eventListener(eventType, eventEnum, eventName, eventData) {
    ...
    const checkField = document.getElementById("check-correct-ric-container");
    if (checkField) {
        if (eventType === 'conn') {
            switch (eventEnum) {
...
if (globalThis.ricConnector) {
    globalThis.ricConnector.setupUpdateManager("2.0.0", `https://updates.robotical.io/live/martyv2/rev{HWRevNo}/current_version.json`, fileDownloader);
    globalThis.ricConnector.setEventListener(eventListener);
}
globalThis.ricPrevData = {};
function formatStatus(name, status, validMs, formatFn, elId) {
...
function addFields(defs, container) {
    defs.forEach(def => {
...
function genStatusBlock(id, elclass, parent) {
...
function component() {
    ...
    // Buttons
    const bleConnDefs = [
        { name: "Disconnect", button: "Disconnect", func: disconnect, params: [] },
    ...
    const wifiConnDefs = [
        { name: "Connect WiFi", button: "Connect", func: connectWiFi, params: [] },
    ];
    const buttonDefs = [
